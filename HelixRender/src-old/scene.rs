...
pub struct Scene {
    ...
    camera_gen: Vec<u32>,         // represents the newest Camera in slot
...
impl Scene {
    ...
    pub fn get_object_matrix(&self, h: &PyObjectHandle) -> Option<[[f32; 4]; 4]> {
        if let Some(obj) = self.resolve_obj(h) {
            Some(obj.transform.get_matrix().to_cols_array_2d())
        } else {
            None
        }
    }
    ...
    pub fn create_camera(&mut self) -> PyResult<PyCameraHandle> {
        let cam = Camera::new();
    ...
    pub fn get_camera_view(&self, h: &PyCameraHandle) -> Result<Vec<Vec3>, String> {
        // To Implement
    }
    ...
    pub fn apply_mesh_to_object(
        &mut self,
        obj_handle: &PyObjectHandle,
        mesh_handle: &PyMeshHandle,
    ) -> PyResult<()> {
        let mesh = self.resolve_mesh(mesh_handle).ok_or_else(|| {
            PyErr::new::<pyo3::exceptions::PyValueError, _>("Invalid object handle")
        })?;
        let obj = self.resolve_obj_mut(obj_handle).ok_or_else(|| {
            PyErr::new::<pyo3::exceptions::PyValueError, _>("Invalid object handle")
        })?;
...
impl Scene {
    ...
    fn resolve_cam(&self, h: &PyCameraHandle) -> Option<&Camera> {
    ...
    fn resolve_cam_mut(&mut self, h: &PyCameraHandle) -> Option<&mut Camera> {
        if self.camera_gen[h.index] == h.generation {
...
impl PyObjectHandle {
    ...
    pub fn new(index: usize, generation: u32) -> Self {
        PyObjectHandle { index, generation }
    }
}
